...
class DiscreteMamba2(nn.Module):
    ...
    def forward(self, u, inference_params=None, output_attentions=False, **kwargs):
        """
        Args:
            u: (B, L, D),
        ...
        if output_attentions:
            M = materialize_mixer(A_log=A_log, B=B, C=C, D=self.D)
            return out[:, :seqlen, :], M
...
def materialize_mixer(A_log, B, C, D):
    ...
    # Change precision of A_log to match B and C
    A_log, B, C = A_log.float(), B.float(), C.float()
