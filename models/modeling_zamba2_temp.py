...
def print_once(msg: str):
    """
    Prints a message only once. This is useful for logging messages that should not be repeated.
    """
    if not hasattr(print_once, "prints"):
        print_once.prints = set()
    if msg not in print_once.prints:
        print_once.prints.add(msg)
        print(msg)

# HEADS_TO_KEEP = [0, 1, 3, 9, 10, 11, 13, 14, 15, 16, 19, 20, 22, 23, 24, 26, 27, 28, 29, 30, 31]
# LAYERS_TO_KEEP = []
ALL_ATTENTION_LAYERS = [6, 11, 17, 23, 29, 35, 41, 47, 53, 59, 65, 71, 77]
# REMOVE_HEADS = {2, 4, 5, 6, 7, 8, 12, 17, 18, 21, 25}
# REMOVE_HEADS = {2, 4, 6, 8, 17, 18, 21, 25}
REMOVE_HEADS = {2, 4, 6, 8, 17, 18, 21, 25}
REMOVE_LAYERS = [47, 59] # 47, 59 are important
def eager_attention_forward(
    ...
    **kwargs,
    ...
    # attn_weights.shape = (batch_size, num_attention_heads, seq_len, seq_len)
    if last_indices is not None and last_indices.shape[1] == 2:
        print_once(f"removing heads: {REMOVE_HEADS}\nkeeping heads: {set(range(32)) - REMOVE_HEADS}")
        print_once(f"removing layers: {REMOVE_LAYERS}\nkeeping layers: {set(ALL_ATTENTION_LAYERS) - set(REMOVE_LAYERS)}")
        for head in REMOVE_HEADS:
            attn_weights[last_indices[:, 0], head, last_indices[:, 1], :] = 0.0
    # if last_indices is not None and last_indices.shape[1] == 2:
    #     attn_weights[last_indices[:, 0], :, last_indices[:, 1]] = 0.0
    # elif last_indices is not None and last_indices.shape[1] == 3:
    #     attn_weights[last_indices[:, 0], :, last_indices[:, 1], last_indices[:, 2]] = 0.0
...
class Zamba2Attention(nn.Module):
    ...
    def forward(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        assert layer_idx in ALL_ATTENTION_LAYERS
        if layer_idx not in REMOVE_LAYERS:
            last_indices = None
...
class Zamba2AttentionDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        last_indices: Optional[torch.LongTensor] = None,
        **kwargs: Unpack[FlashAttentionKwargs],
...
class Zamba2ForCausalLM(Zamba2PreTrainedModel, GenerationMixin):
    ...
    def forward(
        ...
        **loss_kwargs,
        ...
        def entire_last_row(input_ids):
            mask = input_ids != 0
            seq_lengths = mask.sum(dim=1) - 1
            batch_indices = torch.arange(input_ids.size(0), device=input_ids.device)
            last_indices = torch.stack([batch_indices, seq_lengths], dim=1)
            return last_indices

        def selected_last_row(input_ids):
            # targets = torch.tensor([13, 28723, 28741, 28760, 28743, 28757], device=input_ids.device)
            targets = torch.tensor([13, 28723], device=input_ids.device)
            positions = torch.isin(input_ids, targets).nonzero(as_tuple=False)
            # assert positions.size(0) >= 4
            ...
            last_valid_indices = seq_lengths[batch_indices].unsqueeze(1)
            last_indices = torch.cat([batch_indices.unsqueeze(1), last_valid_indices, positions[:, 1].unsqueeze(1)], dim=1)
            return last_indices


        if hasattr(self, "mask_last_row") and self.mask_last_row:
            last_indices = entire_last_row(input_ids)
        else:
            last_indices = None
